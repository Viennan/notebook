...
- [code-ana_codex](./code-ana_codex/)：`openai/codex` 代码仓库分析。当前已完成 agent core 主链路、sandbox/permissions、session 执行调度、multi-agent/subagent、Goal 模式、Responses API 与 Prompt Cache、Prompt 系统与 Compaction 策略、Memory 系统专题报告，覆盖交互式 CLI/TUI 到 core 执行链路、工具执行、approval/sandbox、平台隔离后端、network proxy/approval、thread/session/turn/task 调度、V1/V2 subagent 工具接口、`AgentControl`/mailbox/AgentPath` 架构、review/compact/memory 等 internal subagent 边界与工作时序、长 horizon 任务的持久目标与跨 turn idle 自动续跑，以及 Responses request/stream、WebSocket `previous_response_id` 增量、Responses Lite、prompt 片段组织、local/remote/remote v2 compaction、`prompt_cache_key` 稳定前缀和 `cached_input_tokens` 记账；源码以本 topic 内 submodule 为准。
